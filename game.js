...
function showGameScreen(
    ...
    // =========================
    //
    // movementPath[0] = サイコロを振る前の正式な現在地
    // movementPathの末尾 = 現在の仮移動位置
    //
    // 前のマスへ戻った場合はmovementPathを1つ戻し、
    // その分だけ残り移動数を1つ回復します。
    // =========================
    let movementPath = [];
...
function movePlayer(
    ...
    // =========================
    //
    // 出目の歩数で到達できる「停止可能マス」を
    // すべて強調表示します。
    // プレイヤーが強調されたマスをタップすると、
    // そのマスまで自動的に1マスずつ移動します。
    //
    // ※ 通常サイコロでは、従来の矢印による
    //    1マスごとの手動選択は行いません。
    // =========================
...
// =========================
//
// 画面に描かれている道と同じ接続情報を使います。
// nextの向きだけではなく、現在地へ入っている道も
// getConnectedOptions()で取得するため、前後どちらにも進めます。
// =========================

function showMovementArrows(
    player
) {
    ...
    // =========================
    //
    // movementPathの直前のマスへ戻った場合だけ、
    // 「引き返し」として扱います。
    // その1歩を取り消して残り歩数を1つ戻します。
    // =========================
