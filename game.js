...
function applyMapZoom() {
    ...
    // =========================
    // transformで拡大した分だけ、
    // スクロール可能な余白をレイアウト側にも確保します。
    // これをしないと、見た目だけ拡大して
    // scrollWidth / scrollHeight が増えず、
    // スマホでプレイヤーを中央まで移動できない位置が発生します。
    // =========================

    const baseWidth =
        mapBoard.offsetWidth;

    const baseHeight =
        mapBoard.offsetHeight;
    ...
    if (
        ...
        const extraWidth =
            Math.max(
                0,
                baseWidth * (mapZoom - 1) / 2
            );

        const extraHeight =
            Math.max(
                0,
                baseHeight * (mapZoom - 1) / 2
            );

        mapBoard.style.marginLeft =
            `${extraWidth}px`;

        mapBoard.style.marginRight =
            `${extraWidth}px`;

        mapBoard.style.marginTop =
            `${extraHeight}px`;

        mapBoard.style.marginBottom =
            `${extraHeight}px`;
    }
}
...
// サイコロ移動専用：次へ進めるマスを取得
// =========================
// 通常の移動ルート判定では mapData の next だけを使用します。
// 「前のマスへ戻る」は next の探索には含めません。
// 実際の仮移動中だけ、diceMovementState.history の直前マスを
// 特別な「引き返し」として矢印表示します。
function getDiceForwardOptions(position) {

    const square =
        mapData.find(function (item) {
            return item.id === position;
        });

    if (!square || !Array.isArray(square.next)) {
        return [];
    }

    return square.next.filter(function (nextPosition, index, array) {
        return (
            mapData.some(function (item) {
                return item.id === nextPosition;
            }) &&
            array.indexOf(nextPosition) === index
        );
    });
}
...
function getDiceReachableStopSquares(
    ...
    while (queue.length > 0) {
        ...
        const options =
            getDiceForwardOptions(state.position);

        options.forEach(function (nextPosition) {
            ...
            if (
                ...
                nextPosition === state.previousPosition
            ) {
                return;
            }
            ...
            const nextKey =
                `${state.position}-${nextPosition}-${nextStepsUsed}-${state.previousPosition}`;
...
// 強調されたマスまで移動
// =========================
// 現在位置から「残り歩数ちょうど」で到達できるルートを
// highlightDiceReachableSquares() が渡してきます。
// ここではそのルートを1マスずつ進み、前進するたびに
// 残り歩数を1消費します。
...
// 「行けるかなマス」を強調表示
// =========================
// 重要：ここで使う歩数は「今回の残り歩数」です。
// 開始地点を毎回基準にするのではなく、現在地点から
// 残り歩数ちょうどで到達できる着地マスだけを表示します。
// ただし、探索ルートでは「直前のマスへ引き返す」ことを禁止します。
// 実際に矢印で引き返した場合は history が戻るため、
// その時点の現在地点・残り歩数から再計算されます。
function highlightDiceReachableSquares(
    ...
    reachable.forEach(function (path, squareId) {
...
// =========================
// ・通常の前進：現在マスの next のみ
// ・引き返し：history の直前の1マスだけ
//
// これにより、mapData の next を壊さずに
// 「直前のマスへ戻ったときだけ1カウント回復」を実現します。
function showDiceMovementArrows() {
    ...
    const forwardOptions =
        getDiceForwardOptions(player.position);
    ...
    // 前進方向
    // =========================
    forwardOptions.forEach(function (option) {

        // nextに直前のマスが含まれていても、
        // それは「前進」ではなく「引き返し」として扱います。
    ...
    if (
        ...
        !arrowOptions.some(function (item) {
    ...
    arrowOptions.forEach(function (arrowOption) {

        const option =
            arrowOption.position;

        const destination =
            mapData.find(function (square) {
                return square.id === option;
            });
        ...
        arrowImage.style.pointerEvents = "none";

        arrow.appendChild(arrowImage);

        const dx =
            destination.x - currentSquare.x;
        const dy =
            destination.y - currentSquare.y;

        const angle =
            Math.atan2(dy, dx) * 180 / Math.PI;

        const arrowPosition = 0.35;
        ...
        arrow.style.appearance = "none";
        arrow.style.webkitAppearance = "none";
        ...
        arrow.addEventListener(
            ...
            function () {
                ...
                const currentPreviousPosition =
                    history.length >= 2
                        ? history[history.length - 2]
                        : null;

                const isBacktracking =
                    option === currentPreviousPosition;
                ...
                // 現在地点と残り歩数が変わったので、
                // 「行けるかなマス」を現在地点基準で更新します。
                showDiceMovementArrows();
            }
        );

        mapBoard.appendChild(arrow);
    });
}
...
function centerCurrentPlayerOnMap(
    ...
    function adjustCenter(
        ...
        // 実際の画面上のズレをそのまま使います。
        // mapBoardのscale倍率で割らないことが重要です。
        const offsetX =
            playerCenterX - mapCenterX;

        const offsetY =
            playerCenterY - mapCenterY;
        ...
        const targetScrollLeft =
            mapArea.scrollLeft + offsetX;

        const targetScrollTop =
            mapArea.scrollTop + offsetY;
