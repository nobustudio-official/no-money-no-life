...
function applyMapZoom() {
    ...
    // =========================================
    // CSS transform はレイアウト上の
    // scrollWidth / scrollHeight を増やさない。
    //
    // そのためスマホで拡大すると、
    // マップ端のプレイヤーを中央へ持ってくる
    // ためのスクロール余地が不足する。
    ...
    const extraX =
        Math.max(
            0,
            (baseWidth * mapZoom - baseWidth) / 2
        );

    const extraY =
        Math.max(
            0,
            (baseHeight * mapZoom - baseHeight) / 2
        );
...
// =========================================
// 現在位置から「ちょうどstepsマス」で
// destinationへ到達するルートを1本取得
// =========================================
//
// これは「行けるかなマス」を新しく作る
// 処理ではない。
// サイコロを振った時点で確定した候補のうち、
// 現在の残り歩数で到達できる候補へ移動する
// ためだけに使う。
// =========================================

function getDicePathToDestination(
    startPosition,
    destination,
    steps
) {
    ...
    if (
        ...
        return startPosition === destination
            ? [startPosition]
            : null;
...
// 「行けるかなマス」の母集団は、
// サイコロを振った瞬間に確定した
// initialReachable から変えない。
//
// ただし、プレイヤーが仮移動した後は、
// 現在位置から「残り歩数」で実際に到達できる
// 候補だけを表示する。
//
// これにより、
// Aから3マスで確定した候補の外側へ
...
function highlightDiceReachableSquares(
    ...
    const initialReachable =
        diceMovementState.initialReachable;

    if (
        !initialReachable ||
        initialReachable.size === 0
    ) {
    ...
    initialReachable.forEach(
        function (initialPath, squareId) {
...
// =========================================
//
// path は「現在位置から、残り歩数ちょうど」で
// 選択された候補へ到達するルート。
//
// ルート途中で直前のマスへ戻った場合は、
// その1マス分だけ remainingSteps を回復する。
// =========================================
...
function centerCurrentPlayerOnMap(
    ...
    function adjustCenter(
        ...
        // だけを使う。
        //
        // mapZoom を掛けたり割ったりしない。
        // getBoundingClientRect() が transform後の
        // 実座標を返すため。
        // =========================================
        ...
        // =========================================
        // 実画面上のズレを、そのまま
        // scrollLeft / scrollTop の補正量にする。
        //
        // ここでは mapZoom による再計算をしない。
        // =========================================
        ...
        // =========================================
        // smooth終了後に実座標を再測定。
        // iOS Safari のスクロール・transform
        // タイミング差にも対応する。
        // =========================================

        if (
            currentBehavior === "smooth"
        ) {
